...
1. **不扣真实进度**。水位只是一面镜子，`seconds` 一秒都不会少，也不参与任何结算。
   桌宠上的进度环走的仍是真值——瓶子空了但环还在走，这是「没有被扣掉」的现场证据。
2. **回到白名单立刻退潮**，而且退得比涨得快得多（涨满 8 分钟，退空 45 秒）。
   回头的动作要马上被奖励。
...
- **探测子进程按需启动**：只有「开着 + 勾了至少一个白名单 + 正在跑的专注会话」
  三者同时成立时才拉起来，会话一结束立刻 kill。不专注的时候机器上不该有一个
  常驻进程盯着用户开了什么。
...
- 只占屏幕**下半**（`FLOOD_MAX_RATIO`），不是全屏。一个全屏透明窗常驻画波浪，
  风扇会转起来。
